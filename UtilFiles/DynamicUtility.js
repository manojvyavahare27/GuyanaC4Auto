...
export async function checkAllLocatorVisibility(locators, expect) {
  for (const [index, locator] of locators.entries()) {
    // Safety check
    if (!locator) {
      throw new Error(`❌ Locator at index ${index} is undefined. Check your array.`);
    }

    // Print which locator is being checked
    console.log(`🔍 Checking visibility of locator ${index + 1}/${locators.length}`);

    // Wait until visible (Playwright’s built-in wait)
    await expect(locator).toBeVisible();

    // Log confirmation
    console.log(`✅ Locator ${index + 1} is visible.`);

    // 👇 Add delay so you can visually see execution on the screen
    await new Promise(resolve => setTimeout(resolve, 300)); // 1000 ms = 1 second delay
  }

  console.log("🎉 All locators have been verified for visibility!");
}

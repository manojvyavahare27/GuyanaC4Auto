...
test.describe('New Patient', () => {
  test('Register New Patient', async ({ page }) => {
    ...
    //
      ...
      // await expect(page.getByText('Number & Road required')).toHaveText('Number & Road required')
      // await expect(page.getByText('District is required')).toHaveText('District is required')
      await expect(page.getByText('Country required')).toHaveText('Country required')
     // await expect(page.getByText('Health Region is required')).toHaveText('Health Region is required')
await page.pause()
      await addaddress.enterDestrictSearch();

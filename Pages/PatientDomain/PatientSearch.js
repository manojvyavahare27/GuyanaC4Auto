...
class PatientSearch {
   ...
   async validateMPIField(page, txtbox_MPINumber, options = {}) 
   {
    ...
    async  untickCheckboxIfCheckedJS(page) {
  const checkbox = page.locator(`xpath=${checkbox_IncludeServicePatient}`);
  ...
  if (isChecked) {
    ...
    // Uncheck Include Service Patient checkbox if checked
    async uncheckIncludeServicePatientIfChecked() {
        const isChecked = await this.checkbox_IncludeServicePatient.isChecked();
        if (isChecked) {
            await this.checkbox_IncludeServicePatient.uncheck();
        }
    }

...
test.describe("Database Comparison Add New Referral", () => {
  ...
  test("Add New Referral @Functional @ReferralDomain", async ({ page }) => {
   ...
   await page.pause()
    ...
    // Visibility check for transfer popup locators
    let locatorsTransfer = [
      stockTransferPopUp.LocationToTransferTo,
      stockTransferPopUp.QuantityToTransfer,
      stockTransferPopUp.Transferbutton
    ];
    await checkAllLocatorVisibility(locatorsTransfer, expect);
   ...
   await page.getByTestId('stockControlDrawerHeader').click()


    //DB for Select Transfer quantity

    var sqlQuery = "SELECT sb.stbat_id,sb.stbat_batch_number, cst.sttra_quantity, cst.sttra_request_type, cst.sttra_status FROM c4_stock_transfer cst JOIN  stock_batches sb ON cst.sttra_stbat_id = sb.stbat_id WHERE sb.stbat_id = 22011 ORDER BY cst.sttra_id DESC LIMIT 1;"
...
// Visibility helper for StockTransferPage locators

    const stockTransferlocators = [
        stockTransferPage.requestLocationInput,
       
        stockTransferPage.formularyInput,
        stockTransferPage.transferStatusInput,
        stockTransferPage.requestIdInput,
        stockTransferPage.searchStockItemInput,
        stockTransferPage.startDateInput,
        stockTransferPage.endDateInput,
        stockTransferPage.reloadButton,
        stockTransferPage.sortByInput
    ];
      await checkAllLocatorVisibility(stockTransferlocators, expect);

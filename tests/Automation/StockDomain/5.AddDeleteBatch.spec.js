...
test.describe("Database Comparison Add New Referral", () => {
  ...
  test("Add New Referral @Functional @ReferralDomain", async ({ page }) => {
    ...
    await page.waitForTimeout(2000);   
await page.pause()
    // Create locators array for StockItemFilters
    ...
    await page.pause()
    // Call the visibility check method
...
await page.pause()
   // Create locators array for AddBatch
let locatorsAddBatch = [
    addBatch.suppliername,
    addBatch.manufacturer,
    addBatch.batchNumber,
    addBatch.batchQuantity,
    addBatch.purchaseQuantity,
    addBatch.serialNumber,
    addBatch.sterlisationExpiryDate,
    addBatch.manufacturedDate,
    addBatch.receivedDate,
    addBatch.expiryDate,
    addBatch.retailPrice,
    addBatch.purchaseRate,
    addBatch.saveButton,
...
await page.pause()
  ...
  const fileName = "LocatorJSON/EditBatchPage.json";
   await createPageLocatorJSON(locatorsEditBatch, filePath, fileName);
await page.pause()
    const matched = await compareJsons(fileName,null,jsonData.EditStockItemA[index]);
    if (matched) {
      console.log("\n Front end data matches data from excel sheet\n");

...
test.describe("Database Comparison Add New Referral", () => {
  ...
  test("Add New Stock @Functional @StockDomain", async ({ page }) => {
   ...
   // Array of locators from StockItemFilters.js (use an instance named `stockItemFilters`)
...
await checkAllLocatorVisibility(locatorsStockItemFilters, expect);
   ...
   // create instance first: const addStockItems = new AddStockItemsPage(page);
let locatorsAddStockItems = [
...
await checkAllLocatorVisibility(locatorsAddStockItems, expect);
 ...
 //await page.pause()
   ...
   await checkAllLocatorVisibility(locatorsAddBatch , expect);
   await addStockItems.enterSupplier(jsonData.AddNewStock[0].stbat_supplier)
   await addStockItems.enterManufacturer()
